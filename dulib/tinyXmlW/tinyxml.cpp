...
void TiXmlElement::Print( FILE* cfile, int depth ) const
{
	...
	for ( i=0; i<depth; i++ )
	{
		_ftprintf( cfile, TEXT("    ") );
	}

	_ftprintf( cfile, TEXT("<%s"), value.c_str() );
...
bool TiXmlDocument::LoadFile( const TCHAR* filename )
{
	...
	FILE* file = _tfopen( value.c_str (), TEXT("r") );
	...
	if ( file )
	{
		...
		while( _fgetts( buf, BUF_SIZE, file ) )
		{
			data += buf;
		}
		fclose( file );
...
void TiXmlAttribute::Print( FILE* cfile, int /*depth*/ ) const
{
	...
	if (value.find ('\"') == TIXML_STRING::npos)
		_ftprintf (cfile, TEXT("%s=\"%s\""), n.c_str(), v.c_str() );
	else
		_ftprintf (cfile, TEXT("%s='%s'"), n.c_str(), v.c_str() );
}
...
{
	if ( _stscanf( value.c_str(), TEXT("%d"), ival ) == 1 )
...
{
	if ( _stscanf( value.c_str(), TEXT("%lf"), dval ) == 1 )
		return TIXML_SUCCESS;
	return TIXML_WRONG_TYPE;
}
...
void TiXmlText::Print( FILE* cfile, int /*depth*/ ) const
{
	...
	_ftprintf( cfile, TEXT("%s"), buffer.c_str() );
}
...
{
	_ftprintf (cfile, TEXT("<?xml "));

	if ( !version.empty() )
		_ftprintf (cfile, TEXT("version=\"%s\" "), version.c_str ());
	if ( !encoding.empty() )
		_ftprintf (cfile, TEXT("encoding=\"%s\" "), encoding.c_str ());
	if ( !standalone.empty() )
		_ftprintf (cfile, TEXT("standalone=\"%s\" "), standalone.c_str ());
	_ftprintf (cfile, TEXT("?>"));
}
...
TiXmlAttribute*	TiXmlAttributeSet::Find( const TCHAR * name ) const
{
	...
	for( node = sentinel.next; node != &sentinel; node = node->next )
	{
		if ( lstrcmpi(node->name.c_str(), name) == 0 )
			return node;
	}
	return 0;
}
